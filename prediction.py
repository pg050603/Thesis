...
def get_cmems(date_start, date_end, UN_CMEMS, PW_CMEMS):
    ...
    target_date = date_m.strftime("%Y-%m-%d")
    print(f"Fetching data for midpoint date: {target_date}")

    # 2. Dynamic Dataset Selection
    # The NRT dataset generally holds a rolling 2-year window. 
    two_years_ago = datetime.now() - timedelta(days=365 * 2)

    if date_m < two_years_ago:
        print(f"Date is historical (>2 years ago). Using Multi-Year (Reanalysis)...")
        # Historical Wave Data
        dataset_wav = "cmems_mod_glo_wav_my_0.083deg_PT3H-i"
        
        # Historical Physics (Reanalysis usually has currents in daily files)
        dataset_phy = "cmems_mod_glo_phy_my_0.083deg_P1D-m"
        vars_phy = ["uo", "vo", "thetao", "so", "zos"]

    else:
        print(f"Date is recent. Using Analysis & Forecast (NRT)...")
        # Recent Wave Data
        dataset_wav = "cmems_mod_glo_wav_anfc_0.083deg_PT3H-i"
        
        # CRITICAL FIX: Use the HOURLY dataset (PT1H-m) for recent data.
        # The Daily (P1D-m) dataset often lacks surface currents.
        dataset_phy = "cmems_mod_glo_phy_anfc_0.083deg_PT1H-m"
        
        # Variables for Hourly Surface dataset
        vars_phy = ["uo", "vo", "thetao", "so", "zos"]

    # 3. Create a temporary folder
    download_dir = "cmems_data"
    if not os.path.exists(download_dir):
        os.makedirs(download_dir)

    # 4. Download Wave Data
    print(f"Downloading Wave data from {dataset_wav}...")
    try:
    ...
    # 5. Download Physics Data
    print(f"Downloading Physics data from {dataset_phy}...")
    try:
        copernicusmarine.subset(
            dataset_id=dataset_phy,
            variables=vars_phy, # Uses the correct list defined above
            start_datetime=f"{target_date} 00:00:00",
            end_datetime=f"{target_date} 23:59:59",
            output_directory=download_dir,
        ...
        print(f"Error downloading Physics data: {e}")

    # 6. Load the data
    try:
        ds_w = xr.open_dataset(f"{download_dir}/wave_data.nc")
        ds_p = xr.open_dataset(f"{download_dir}/physics_data.nc")
        return (ds_w, ds_p)
    except FileNotFoundError:
        print("CRITICAL ERROR: Data files were not downloaded.")
        return None, None
